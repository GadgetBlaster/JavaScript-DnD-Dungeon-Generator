...
export default ({ assert, describe, it }) => {
    describe('#dot', () => {
        describe('given a falsy `isOk` option', () => {
            it('should return a span with the `dot-fail` class', () => {
                assert(dot({ isOk: false })).equals('<span class="dot-fail"></span>');
            });
        });

        describe('given a truthy `isOk` option', () => {
            it('should return a span with the `dot-ok` class', () => {
                assert(dot({ isOk: true })).equals('<span class="dot-ok"></span>');
            });
        });
    });
